...
public sealed class ReservationRepository : IReservationRepository
{
    ...
    public async Task<Reservation?> GetByIdAsync(Guid id, CancellationToken cancellationToken)
    {
        return await _dbContext.Reservations
            .FirstOrDefaultAsync(reservation => reservation.Id == id, cancellationToken: cancellationToken);
    }

    public async Task<IReadOnlyList<Reservation>> GetByEventIdAsync(Guid eventId, CancellationToken cancellationToken)
    {
        return await _dbContext.Reservations
    ...
    public async Task<bool> ExistsByConfirmationCodeAsync(string confirmationCode, CancellationToken cancellationToken)
    {
        var candidateCode = ConfirmationCode.FromExisting(confirmationCode);

        return await _dbContext.Reservations
            .AnyAsync(reservation => reservation.ConfirmationCode == candidateCode, cancellationToken: cancellationToken);
    }
